...
const inventorObjs = inventors.find((inventor) => inventor.first === "Ada");
console.log(inventorObjs);
...
const peopleNames = people.map((peopleName) => {
  return peopleName.split(", ").reverse().join(" ");
});
...
const items = data.reduce((runningTally, item) => {
  runningTally[item] = runningTally[item] ? runningTally[item] + 1 : 1;
  return runningTally;
}, {});
console.log(items);
...
const olderNineteen = devs.some((dev) => {
...
const nineteenOlder = devs.every((dev) => {
  const age = new Date().getFullYear() - dev.year;
  return age >= 19;
});
